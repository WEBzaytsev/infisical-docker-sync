...
6. Если `.env` уже записан, но pull или recreate завершился ошибкой, агент сохраняет pending-задачу и повторяет recreate на следующем интервале. `.env` не откатывается; удалённые ключи сохраняются для повторной попытки даже после перезапуска агента.
