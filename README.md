...
4. При изменении секретов:
   ...
   - Перезапускает связанный Docker-контейнер согласно указанной политике:
     ...
     - `compose`: Умный режим для контейнеров, управляемых через docker-compose
  
### Политики перезагрузки контейнеров
...
- **compose**: Интеллектуальное определение, запущен ли контейнер через docker-compose. Если да, автоматически запускает `docker compose up -d` в нужной директории. **Рекомендуется для контейнеров, управляемых через docker-compose**.
